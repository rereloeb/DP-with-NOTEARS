...
class LBFGSBScipy1(torch.optim.Optimizer):

    def __init__(self, params):
        defaults = dict()
        super(LBFGSBScipy1, self).__init__(params, defaults)
...
#param bounds for the L-BFGS-B algorithm
        bounds = self._gather_flat_bounds()
#Parameters of the L-BFGS-B algorithm
        optiondict = {'disp':False, 'maxcor':3, 'maxiter':1000}
#scipy library called to perform the optimization inside the step method of a pytorch optimizer
        sol = sopt.minimize(wrapped_closure, initial_params, method='L-BFGS-B', jac=True, bounds=bounds, options=optiondict)
...
class LBFGSBScipy2(torch.optim.Optimizer):

    def __init__(self, params):
        defaults = dict()
        super(LBFGSBScipy2, self).__init__(params, defaults)
...
#scipy library called to perform the optimization inside the step method of a pytorch optimizer
#        sol = sopt.minimize(wrapped_closure, initial_params, method='L-BFGS-B', jac=True, bounds=bounds, options=optiondict)
        sol = sopt.minimize(wrapped_closure, initial_params, method='L-BFGS-B', jac=True, options=optiondict)
        final_params = torch.from_numpy(sol.x)
        final_params = final_params.to(torch.get_default_dtype())
        self._distribute_flat_params(final_params)


def LBFGSBScipy(params, boxpenalty):
    if boxpenalty:
        return LBFGSBScipy1(params)
    else:
        return LBFGSBScipy2(params)
