...
def get_data_loaders(minibatch_size, microbatch_size, iterations, drop_last=True):
    ...
    def minibatch_loader(dataset):
        return torch.utils.data.DataLoader( dataset, batch_sampler=IIDBatchSampler(dataset, minibatch_size, iterations) )

    def microbatch_loader(minibatch):
        return torch.utils.data.DataLoader( minibatch,
